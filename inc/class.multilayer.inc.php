...
//        }
//        
//        //REVISA LAS BOLSAS ACTUALES EN STOCK Y LAS QUE SE NECESITAN PARA HACER EL BATCH
//        $sql = "SELECT stock_material_id, material_name, material_grade, bags, ROUND((kg/kgs_bag)*". $total .",2) AS bags_needed FROM `multilayer_formulas`
//                NATURAL JOIN materials
//                INNER JOIN stock_materials ON multilayer_formulas.material_id = stock_materials.material_id
//                WHERE layer = ". $layer ." AND machine_id = 2;";
//        $update = "";
//        if($stmt = $this->_db->prepare($sql))
//         {
//            $stmt->execute();
...
//                if($row['bags']<$row['bags_needed'])
//                {
//                    echo '<strong>ERROR</strong> The <strong>'. $total .'</strong> batches were not added to the layer: <strong>'. $this->giveLayername($layer) .'</strong>. Because there is not enought material <strong>'. $row['material_name'] .' - '. $row['material_grade'] .'</strong> in stock. <br> There are <strong>'. $row['bags'] .'</strong> bags in stock, and you need <strong>'. $row['bags_needed'] .'</strong> bags.';
//                    return false;
//                }
//                // VA CREANDO EL UPDATE PARA CAMBIAR DESPUES EL NUMERO DE BOLSAS DE STOCK_MATERIALS
//                else
//                {
//                    $newbags = $row['bags']-$row['bags_needed'];
//                    $update = $update . "UPDATE  `stock_materials` SET `bags` = ".$newbags." WHERE `stock_material_id` = ". $row['stock_material_id']. "; ";
//                }
//            }
//                    
//        
//            //INSERT THE NUMBER OF BATCHES IN THE DAY DECREASES THE BAGS FROM THE STOCK MATERIALS 
//            $sql = "INSERT INTO  `multilayer_batches`(`batch_id`,`date_batch`,`shift`,`layer`,`quantity`,`user_id`)
//                VALUES(NULL,". $date . ",:shift,:layer,:total,:user); 
//                UPDATE  `multilayer_batches_stock`
//                SET `kgs` = kgs + (SELECT SUM(kg)*". $total ."
//                FROM  `multilayer_formulas` WHERE layer = :layer)
//                WHERE layer = :layer;
//                ". $update;
//
//            try
...
//                    $stmt->execute();
//                    $stmt->closeCursor();
//                    echo '<strong>SUCCESS!</strong> The <strong>'. $total .'</strong> batches were successfully added to the layer: <strong>'. $this->giveLayername($layer) .'</strong> for the shift: <strong>'. $this->giveShiftname($shift) .'</strong>';
//                    return TRUE;
//                } 
//            catch (PDOException $e) {
//                if ($e->getCode() == 23000) {
//                    echo '<strong>ERROR</strong> The layer: <strong>'. $this->giveLayername($layer) .'</strong> has already register the number of batches for the shift: <strong>'. $this->giveShiftname($shift) .'</strong>.<br>';
//                } 
//                else {
//                    echo '<strong>ERROR</strong> Could not insert the number of batches into the database. Please try again.<br>'. $e->getMessage();
//                }
//                return FALSE;
//            }
//        }
...
//    /**
//     * Loads the table of all the batches
//     * This function outputs <tr> tags with batches
//     * Parameter= ID of the shift ALL DAY=0 MORNING=1 NIGHT=2
//     */
//    public function giveBatches($x)
//    {
//        $date = "date_batch = CURRENT_DATE()";
//        if(!empty($_POST['dateSearch']))
//        {
//            $myDateTime = DateTime::createFromFormat('d/m/Y', $_POST['dateSearch']);
//            $newDateString = $myDateTime->format('Y-m-d');
//            $date = "date_batch BETWEEN '". $newDateString ." 00:00:00' AND '". $newDateString ." 23:59:59'";
//        }
//    
//        $sql = "SELECT layer, SUM(quantity) AS qty FROM `multilayer_batches` WHERE ". $date ." GROUP BY layer ORDER BY layer";
//
//        if($x != 0)
//        {
//            $sql = "SELECT layer, quantity AS qty FROM `multilayer_batches` WHERE ". $date ." AND shift = ". $x ." ORDER BY layer";
//        }
//                
//        if($stmt = $this->_db->prepare($sql))
//        {
...
//                    <td class="text-right">'. number_format($outer,0,'.',',') .'</td>
//                    <td class="text-right">'. number_format($middle,0,'.',',') .'</td>
//                    <td class="text-right">'. number_format($inner,0,'.',',') .'</td>
//                    <td style="text-align:center">'. number_format($total,0,'.',',') .' batches</td>
//                   </tr>';
//            
//            $stmt->closeCursor();
//            
...
//        }
//        
//        $sql = "SELECT layer, SUM(kg) FROM `multilayer_formulas` GROUP BY layer ORDER BY layer";
//        
//        if($stmt = $this->_db->prepare($sql))
//        {
//            $stmt->execute();
//            
//            $row = $stmt->fetch();
//            $outerkg = $outer * $row['SUM(kg)'];
//            
//            $row = $stmt->fetch();
//            $middlekg = $middle * $row['SUM(kg)'];
//            
//            $row = $stmt->fetch();
//            $innerkg = $inner * $row['SUM(kg)'];
//            
//            $totalkg  = $outerkg + $middlekg + $innerkg;
//            
//             echo '<tr>
//                    <th style="text-align:center">Total kgs</th>
//                    <td class="text-right">'. number_format($outerkg,1,'.',',') .'</td>
//                    <td class="text-right">'. number_format($middlekg,1,'.',',') .'</td>
//                    <td class="text-right">'. number_format($innerkg,1,'.',',') .'</td>
//                    <td style="text-align:center">'. number_format($totalkg,0,'.',',') .' kgs</td>
//                   </tr>';
//            
//            echo '<tr>
//                    <th style="text-align:center">Total bags</th>
//                    <td class="text-right">'.  $this->giveBags($outerkg) .'</td>
//                    <td class="text-right">'.  $this->giveBags($middlekg) .'</td>
//                    <td class="text-right">'.  $this->giveBags($innerkg) .'</td>
//                    <td style="text-align:center">'.  $this->giveBags($totalkg) .'</td>
//                   </tr>';
//            
//            $stmt->closeCursor();
//            
...
//        }
//        
//        $sql = "SELECT material_name, material_grade, outer_l.kg AS outer_kg, middle_l.kg AS middle_kg, inner_l.kg AS inner_kg
//FROM materials 
//INNER JOIN `multilayer_formulas` outer_l ON materials.material_id = outer_l.material_id AND outer_l.layer=1
//LEFT JOIN `multilayer_formulas` middle_l ON materials.material_id = middle_l.material_id AND middle_l.layer=2
//LEFT JOIN `multilayer_formulas` inner_l ON materials.material_id = inner_l.material_id AND inner_l.layer=3  
//
//UNION
//
//SELECT material_name, material_grade, outer_l.kg AS outer_kg, middle_l.kg AS middle_kg, inner_l.kg AS inner_kg
//FROM materials 
//INNER JOIN `multilayer_formulas` middle_l ON materials.material_id = middle_l.material_id AND middle_l.layer=2
//LEFT JOIN `multilayer_formulas` outer_l ON materials.material_id = outer_l.material_id AND outer_l.layer=1
//LEFT JOIN `multilayer_formulas` inner_l ON materials.material_id = inner_l.material_id AND inner_l.layer=3  
//
//UNION
//
//SELECT material_name, material_grade, outer_l.kg AS outer_kg, middle_l.kg AS middle_kg, inner_l.kg AS inner_kg
//FROM materials 
//INNER JOIN `multilayer_formulas` inner_l ON materials.material_id = inner_l.material_id AND inner_l.layer=3  
//LEFT JOIN `multilayer_formulas` outer_l ON materials.material_id = outer_l.material_id AND outer_l.layer=1
//LEFT JOIN `multilayer_formulas` middle_l ON materials.material_id = middle_l.material_id AND middle_l.layer=2;";
//        
//        if($stmt = $this->_db->prepare($sql))
...
//                                        <th width="15%" class="text-right">Bags</th>
//                                    </tr>
//                                </thead>
//                   <tbody>';
//            $stmt->execute();
...
//                $GRADE = $row['material_grade'];
//                $OUTERM = 0;
//                if(!is_null($row['outer_kg']))
//                {
//                    $OUTERM = $outer * $row['outer_kg'];
//                }
//                $MIDDLEM = 0;
//                if(!is_null($row['middle_kg']))
//                {
//                    $MIDDLEM = $middle * $row['middle_kg'];
//                }
//                $INNERM = 0;
//                if(!is_null($row['inner_kg']))
//                {
//                    $INNERM = $inner * $row['inner_kg'];
//                }
//                $TOTALM = $OUTERM + $MIDDLEM + $INNERM;
//                echo '<tr>
//                        <td style="text-align:center"><b>'. $MATERIAL .'</b>&nbsp - &nbsp'. $GRADE .'</td>
//                        <td class="text-right">'. number_format($OUTERM,1,'.',',') .'</td>
//                        <td class="text-right">'. number_format($MIDDLEM,1,'.',',') .'</td>
//                        <td class="text-right">'. number_format($INNERM,1,'.',',') .'</td>
//                        <td class="text-right">'. number_format($TOTALM,1,'.',',') .'</td>
//                        <td class="text-right">'.  $this->giveBags($TOTALM) .'</td>
//                       </tr>';
//            }
//            $stmt->closeCursor();
//        }
...
//	/**
//     * Loads the table of all the stock of the mixed materials
//     * This function outputs <tr> tags with stock of mixed materials
//     */
//    public function stockBatches()
//    {
//        $sql = "SELECT `multilayer_batches_stock`.`layer`,
//                    `multilayer_batches_stock`.`kgs`
//                FROM  `multilayer_batches_stock`;";
//        $totalKgs = 0;
//        if($stmt = $this->_db->prepare($sql))
//        {
//            $stmt->execute();
...
//                 
//                echo '<tr>
//                        <td>'. $this->giveLayername($LAYER) .'</td>
//                    <td class="text-right">'. number_format($KGS,2,'.',',') .'</td>
//                        <td>'. $this->giveBags($KGS) .'</td>
//                    </tr>';
//                }
//            
//            $stmt->closeCursor();
...
//                <tr class="active">
//                  <th>ALL LAYERS</th>
//                    <td class="text-right">'. number_format($totalKgs,2,'.',',') .'</td>
//                  <th>' . $this->giveBags($totalKgs) .'</th>
//                </tr>
//              </tfoot>';
//        }
//        else
...
//	/**
//    * Checks and inserts the rolls WHEN THERE IS BATCHES
//    *
//    * @return boolean true if can insert false if not
//    */
...
//                FROM  `settings` 
//                WHERE location_id=2 AND name_setting='680cone';";
//        if($stmt = $this->_db->prepare($sql))
//        {
//            $stmt->execute();
//            while($row = $stmt->fetch())
...
//        $sql = "SELECT `settings`.`value_setting`
//                FROM  `settings` 
//                WHERE location_id=2 AND name_setting='1010cone';";
//        if($stmt = $this->_db->prepare($sql))
//        {
//            $stmt->execute();
//            while($row = $stmt->fetch())
...
//        
//        //REVISA LAS BOLSAS ACTUALES EN STOCK Y LAS QUE SE NECESITAN PARA HACER EL BATCH
//        $sql = "SELECT layer, kgs FROM `multilayer_batches_stock`;";
//        $update = "";
//        if($stmt = $this->_db->prepare($sql))
//         {
//            $stmt->execute();
...
//                if($row['layer']==1)
//                {
//                    $kgsneeded = $totaln * 0.3125;
//                }
//                else if($row['layer']==2)
//                {
//                    $kgsneeded = $totaln * 0.375;
//                }
//                else if($row['layer']==3)
//                {
//                    $kgsneeded = $totaln * 0.3125;
...
//                if($row['kgs']<$kgsneeded)
//                {
//                    echo '<strong>ERROR</strong> The rolls were not added to the system, because there is not enought mixed material for <strong>'. $this->giveLayername($row['layer']) .'</strong> in stock. <br> There are <strong>'. $row['kgs'] .'</strong> kgs in stock, and you need <strong>'. $kgsneeded .'</strong> kgs.';
//                    return false;
//                }
//                // VA CREANDO EL UPDATE PARA CAMBIAR DESPUES EL NUMERO DE KGS DE MULTILAYER_BATCHES_STOCK
//                else
//                {
//                    $newkgs = $row['kgs']-$kgsneeded;
//                    $update = $update . "UPDATE `multilayer_batches_stock` SET `kgs` = ". $newkgs ." WHERE `multilayer_batches_stock`.`layer` = ". $row['layer'] .";";
//                }
//            }
//                    
...
//            $sql = "INSERT INTO `multilayer_rolls` (`multilayer_rolls_id`,`date_roll`,`rollno`,`shift`,`size`,`gross_weight`,`net_weight`,`user_id`,`status_roll`,`dyne_test`,`waste_printing`) VALUES (NULL, '". $datetime."', '".$rollno."-A', ". $shift .", '1', ". $sw .", ". $sn .", ". $_SESSION['Userid'] .",0,1,NULL),(NULL, '". $datetime."', '".$rollno."-B', ". $shift .", '2', ". $bw .", ". $bn .", ". $_SESSION['Userid'] .",0,1,NULL), (NULL, '". $datetime2."', '".$rollno."-C', ". $shift .", '1', ". $sw2 .", ". $sn2 .", ". $_SESSION['Userid'] .",0,1,NULL),(NULL, '". $datetime2."', '".$rollno."-D', ". $shift .", '2', ". $bw2 .", ". $bn2 .", ". $_SESSION['Userid'] .",0,1,NULL);
//                ". $update;
//            try
//            {   
//                $this->_db->setAttribute( PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION );
