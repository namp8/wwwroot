...
?>
 ...
 <?php
    if( empty($_POST['report']) or $_POST['report'] == 1 )
    {
		...
		$macchi->reportEfficiency();        
		echo '</table>';
    }     
    else if( empty($_POST['report']) or $_POST['report'] == 2 )
    {
        echo '<table class="table table-bordered table-hover" id="dataTable" width="100%" cellspacing="0"  >';
        $macchi->reportProduction();
		echo '</table>';
    }  
    ...
    {
        $macchi->reportMaterialConsumption();
    }  
...
?>
    ...
    <script>
        ...
        function selectReport(id) {
            ...
            if(id ==1)
            {
                name = "Macchi - Efficiency Report";
            }
            else if (id == 2)
            {
                name = "Macchi - Production Report";
            }
            else if (id == 3)
            {
                name = "Macchi - Waste Report";
            }
            else if (id == 4)
            {
                name = "Macchi - Raw Material Consumption Report";
            }
			else if (id == 5)
            {
                name = "Macchi - Short Fall and Downtime Report";
            }
            else
            {
                name = "Macchi - Raw Material Stock Details";
            }
					...
					for (var y = 0; y < table.rows.length; y++) {
						...
						for (var x = 0; x < table.rows[y].cells.length; x++) {
							if (y == 0 || y == table.rows.length-1) {
								bdy[y][x] = {
									text: table.rows[y].cells[x].innerHTML.replace(/<br>/g,"\n"),
									style: 'tableHeader'
								};
							} else {
